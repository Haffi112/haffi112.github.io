var width = document.body.clientWidth; //d3.select("#simulation").node().getBoundingClientRect().width;
var height = document.body.clientHeight;

var input = {"x":width/4,"y":height/10};
var neuron = {"x":3*width/4,"y":height/10};
var neuronradius = width/100;;

var a, b, c, d, slowdown, resolution;

var fill = d3.scale.category20c();

var delay = 1*slowdown/resolution;
...
function update_parameters(setting={}){
  ...
  slowdown = parseFloat(document.getElementById('slowdown').value);
  resolution = parseFloat(document.getElementById('resolution').value);
  delay = 1*slowdown/resolution;
}
...
function add_play_button() {
  ...
  play.append("circle")
    ...
    .attr("transform", "translate(" + input.x + "," + input.y + ")")
    .on("mousedown", md)
    .on("mouseup", mup);
}
